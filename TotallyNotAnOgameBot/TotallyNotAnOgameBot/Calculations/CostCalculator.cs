...
namespace TotallyNotAnOgameBot.Calculations
{
    ...
    {
        static public readonly Dictionary<Building.Type, Cost> BuildingsDic
        = new Dictionary<Building.Type, Cost>
        {
        ...
        static private readonly Dictionary<Defense.Type, Cost> DefensesDic
        = new Dictionary<Defense.Type, Cost>
        {
        ...
        static private readonly Dictionary<Resarch.Type, Cost> ResarchDic
        = new Dictionary<Resarch.Type, Cost>
        {
        ...
        static private readonly Dictionary<Spaceships.Type, Cost> SpaceshipDic
        = new Dictionary<Spaceships.Type, Cost>
        {
        ...
        {
            DefensesDic.TryGetValue(type, out Cost cost);
                cost.dsMultiplication(quantity);
            return cost;
        }

        static public Cost getResarchCost(Resarch.Type type, int level)
        {
            ResarchDic.TryGetValue(type, out Cost cost);
            if (type == Resarch.Type.GravitonTechnology)
            {
                cost.rbMultiplication(3, level);
            }else
            {
                cost.rbMultiplication(2, level);
            }
        ...
        {
            SpaceshipDic.TryGetValue(type, out Cost cost);
                cost.dsMultiplication(quantity);
            return cost;
        }
    }
